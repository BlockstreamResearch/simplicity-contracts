...
impl PartialPset {
    ...
    #[must_use]
    pub fn inp_tx_out_sec(&self) -> Option<HashMap<usize, TxOutSecrets>> {
        self.inp_tx_out_sec.clone()
    }

    #[must_use]
    pub fn spent_tx_outs(&self) -> Vec<TxOut> {
        self.spent_tx_outs.clone()
    }

    /// Calculates the transaction fee based on the fee rate and estimated transaction weight.
    /// Can be used for manual tx
    ///
    /// This method creates a temporary copy of the PSET, blinds it if needed,
    /// extracts and signs the transaction to measure its actual weight,
    /// then calculates the fee using the provided fee rate.
    ///
    /// # Arguments
    ///
    /// * `fee_rate` - Fee rate in satoshis per 1000 virtual bytes (sats/kvb)
    /// * `network` - The Simplicity network to use for signing and deriving policy assset
    /// * `signer` - A one-time signer implementation
    ///
    /// # Errors
    ///
    /// Returns [`TransactionBuildError`] if blinding, transaction extraction, or signing fails.
    pub fn calculate_fee(
        ...
        let weight = {
            ...
            if let Some(inp_tx_out_sec) = &self.inp_tx_out_sec {
                temp_pset.blind_last(&mut thread_rng(), secp256k1::SECP256K1, inp_tx_out_sec)?;
            }

            let tx = temp_pset.extract_tx()?;
            let signed_tx = signer(network, tx, &self.spent_tx_outs)
                .map_err(TransactionBuildError::PsetSign)?;
    ...
    pub fn finalize(
        ...
        match self.fee_amount {
            None => {
                ...
                let fee = self.calculate_fee(fee_rate, network, signer.clone())?;
                ...
                if policy_change == 0 {
                    ...
                    self.pset.remove_output(n_outputs - 2);
                } else {
                    let n_outputs = self.pset.n_outputs();
                    let outputs = self.pset.outputs_mut();
