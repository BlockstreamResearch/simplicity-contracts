...
mod options_tests {
    ...
    fn setup_funding_scenario() -> Result<FundingTestContext> {
        ...
        let option_tx_out = final_tx.output[0].clone();
        let grantor_tx_out = final_tx.output[1].clone();
    ...
    fn test_options_funding_path() -> Result<()> {
        ...
        let witness_values = build_option_witness(ctx.branch, Some(&ctx.extra_inputs));
