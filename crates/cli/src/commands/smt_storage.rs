...
use contracts::smt_storage::{
    DEPTH, SMTWitness, SparseMerkleTree, finalize_get_storage_transaction,
    ...
    pub async fn handle(&self) -> anyhow::Result<()> {
        match self {
            Self::GetStorageAddress {
                collateral_utxo: collateral_utxo_outpoint,
                collateral_amount,
                fee_amount,
                storage_bytes,
                path,
                ...
                let collateral_tx_out = fetch_utxo(*collateral_utxo_outpoint).await?;

                let pst = contracts::sdk::build_smt_mint(
                    &public_key,
                    (*collateral_utxo_outpoint, collateral_tx_out.clone()),
                    *collateral_amount,
                    *fee_amount,
                    storage_bytes,
                    *path,
                ...
                let utxos = vec![collateral_tx_out];
            ...
            Self::TransferFromStorageAddress {
                ...
                let witness = SMTWitness::new(&public_key.serialize(), storage_bytes, &merkle_data);
