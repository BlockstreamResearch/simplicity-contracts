...
use simplicityhl::elements::{AssetId, Transaction};
...
mod tests {
    ...
    fn test_verify_issuance_valid() -> Result<(), String> {
        let script_pubkey = Script::from_hex(
            "51203451c2c04047ab5f2d3eb747773e5a9756c10eb9bf31326bd16a6533142f1ace",
        )
        .unwrap();

        let tx_hex = include_str!("../../tests/data/tx_with_issuance_token.hex");
        ...
        let constraints = IssuanceTxConstraints {
            inputs: vec![
                IssuanceInputConstraints {
                    input_idx: 0,
                    issuance_destination: None,
                    reissuance_destination: Some((script_pubkey.clone(), 1, None)),
                },
                IssuanceInputConstraints {
                    input_idx: 1,
                    issuance_destination: None,
                    reissuance_destination: Some((script_pubkey, 1, None)),
                },
            ],
            ..Default::default()
        };
